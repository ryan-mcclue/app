...
INTERNAL void
json_example_usage(void)
{
  // IMPORTANT(Ryan): At 10mil. get killed by linux OOM (verified with gdb SIGKILL and dmesg)
  // This is because although we are clearing temp arena, linux sees we have written to these areas
  // and so isn't aware that they are in effect 'free'
  //gen_json_data(str8_lit("data.json"), MILLION(1));
  //MemArenaTemp temp = mem_arena_temp_begin(NULL, 0);
  //MemArena *temp_arena = temp.arena;


/*   String8 json_data = str8_read_entire_file(arena, str8_lit("data.json"));
  JsonTokenArray a;
  PROFILE_BANDWIDTH("lexing", json_data.size)
  {
    a = lex_json(arena, json_data);
  }
  //for (u32 i = 0; i < a.count; i += 1) print_json_token(a.tokens[i]);
  JsonElementNode *root;
  PROFILE_BANDWIDTH("parsing", a.count * sizeof(JsonToken))
  {
    root = parse_json(a, arena);
  }
  JsonElementNode *data_node = lookup_label_json(root, str8_lit("data"));
  JsonDataArray data_array = parse_json_data(arena, data_node);
   for (u32 i = 0; i < data_array.count; i += 1)
  {
    Vector4 *v = &data_array.data[i];
    printf("w0: %.2f, d0: %.2f, w1: %.2f, d1: %.2f\n", v->x, v->y, v->z, v->w);
  }   */
}
